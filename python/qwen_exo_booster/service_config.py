...
SERVICE_SETTINGS = (
    ...
    _setting(
        ...
        "capacity",
        "内部任务并发",
        "作用：语义裁判、自我提问等后台任务的并发上限。用途：限制内部推理对主请求的干扰。推荐值：8（过高会在 hybrid 模型上抢占 mamba 状态槽、驱逐用户会话前缀，导致下一轮全量重算）。",
        "integer",
        8,
        minimum=1,
        maximum=128,
    ...
    _setting(
        "qwen_exo_internal_mamba_reserve",
        "capacity",
        "用户会话 Mamba 保留槽",
        "作用：为在跑的用户会话保留、内部后台任务永不占用的 mamba 状态槽底线。用途：阻止 fan-out 裁判任务通过“计划驱逐用户状态”通过准入，从而保住用户前缀缓存。推荐值：6（约两个满上下文的 3× COW）。设 0 关闭保护。",
        "integer",
        6,
        minimum=0,
        maximum=64,
    ...
    _setting(
        ...
        "memory",
        "最大候选数",
        "作用：限制初始检索及非 QK 补充候选数量；通过预过滤的 Attention-Q×K 候选全部进入语义准入，首轮全拒绝时再进行一次有界扩张。弱分数候选仍按 fail-closed 规则直接放弃。用途：控制召回范围与裁判耗时（候选越多，每轮起的语义裁判 fan-out 越多，越易在 hybrid 模型上抢 mamba 槽）。推荐值：4。",
        "integer",
        4,
        minimum=1,
        maximum=64,
    ...
    _setting(
        "qwen_exo_pmi_gate_mode",
        "memory",
        "PMI 裁判门",
        "作用：裁判前用目标模型一次前向为每个候选记忆的规则卡开头打分，比较“在当前问题下”与“在中性问题下”的对数概率差（PMI）。所有候选都不高于阈值时直接跳过裁判且不附着记忆，与裁判全拒的结果一致但省去 1 到 2 轮生成；任何打分失败都回退到裁判。实测无关请求首 token 从约 15 秒降到 3 到 4 秒，相关请求不受影响。推荐值：启用。",
        "string",
        "active",
        choices=("off", "active"),
        choice_labels={"off": "关闭", "active": "启用"},
    ...
    _setting(
        "qwen_exo_pmi_gate_threshold",
        "memory",
        "PMI 裁判门阈值",
        "作用：候选记忆的 PMI（每 token 平均对数概率增益）不高于此值即视为无关。0 表示只拦“问题没有让任何记忆更可预期”的请求；实测相关记忆 PMI 在 0.09 以上，无关请求多为负值或 0.05 以下，调到 0.05 到 0.06 可拦更多无关请求但风险略增。推荐值：0。",
        "number",
        0.0,
        minimum=-1,
        maximum=1,
    ...
    _setting(
        "qwen_exo_observer_surprisal_sustain",
        "observer",
        "连续漂移确认",
        "作用：Mid-Think 触发前，局部窗口需要连续多少个 token 保持高于阈值。用途："
        "把单点尖峰误触发改成连续漂移确认，值越大越保守。1 表示单点即触发。推荐值：4。",
        "integer",
        4,
        minimum=1,
        maximum=64,
    ...
    _setting(
        "qwen_exo_reasoning_cutoff_question_count",
        "observer",
        "截断自检问题数",
        "作用：think 生成超出「最大推理 token」被截断时，先注入这么多条具体、可执行的"
        "自检问题（点名要验证的假设 / 更直接的路径 / 尚未核对的事实），再写 stop-and-go "
        "收束语强制结束思考。用途：让被截断的推理在收尾前快速纠偏。0=关闭问题、仅保留"
        "原截断行为。推荐值：3。",
        "integer",
        3,
        minimum=0,
