...
_COMPATIBILITY_GUIDANCE = (
    ...
    "verified Dense 27B or MoE 35B-A3B Qwen3_5* runtime structures"
)
...
def validate_qwen_exo_config(config: Any) -> str:
    ...
    if float(_config_value(text, "partial_rotary_factor", 0.0) or 0.0) != 0.25:
...
def validate_qwen_exo_model_path(model_path: Path | str) -> str:
    ...
    return validate_qwen_exo_config(config)
...
class ModelIdentity:
    ...
    def from_path(cls, model_path: Path | str) -> ModelIdentity:
        ...
        if not index_path.is_file():
            raise FileNotFoundError(f"Model weight index was not found: {index_path}")
